...
class IntelligentReprocessor:
        ...
        with self.lock:
            if unknown_signal.retry_count < self.max_retries:
                ...
                if self.db:
                    ...
                    logger.warning("No database connection - unknown signal not saved")
    ...
    def _retry_with_bit_flip_correction(self, signal: UnknownSignal, decoder) -> Optional[TPMSSignal]:
        ...
        if signal.baud_rate and abs(signal.baud_rate - 19200) < 2000:
            bits = decoder._extract_bits_from_samples(signal.raw_samples, signal.baud_rate)
        ...
        try:
            ...
            for k in set(labels):
                ...
                if len(cluster_signals) >= self.min_samples_for_discovery:
                    ...
                    logger.info(f"Physics-based discovery: {protocol_name} from {len(cluster_signals)} signals")
...
class TPMSDecoder:
    def __init__(self, sample_rate: int, db_connection=None):
        ...
        if db_connection:
            if hasattr(db_connection, 'insert_unknown_signal'):
                ...
                self.db = db_connection
                logger.warning("Received connection object instead of database object")
        else:
            self.db = None
    ...
    def _start_evolution_loop(self):
        ...
        def evolution_worker():
            ...
            while True:
                try:
                    # Reprocess every 90 seconds
                    if self.reprocessor and reprocess_counter % 3 == 0:
    ...
    def process_samples(self, iq_samples: np.ndarray, frequency: float) -> List[TPMSSignal]:
        """Process IQ samples and attempt to decode TPMS signals"""
        ...
        for protocol_name in protocol_order:
            ...
            if decoded:
                ...
                p_str = f"{decoded.pressure_psi:.1f} PSI" if decoded.pressure_psi is not None else "PSI=?"
                t_str = f"{decoded.temperature_c:.1f}°C" if decoded.temperature_c is not None else "T=?"
        ...
        # If decoding failed, queue for intelligent reprocessing
        if decoded_any is None:
            ...
            if self._failed_count % 100 == 0:
                ...
                logger.info(
                    f"{self._failed_count} signals failed | Success rate: {success_rate:.1f}% | Reprocessed: {self._reprocessing_success}")
    ...
    def _blind_feature_extraction(self, iq_samples: np.ndarray, frequency: float,
        ...
        try:
            # IQ samples are already complex (analytic)
            analytic = iq_samples
            ...
            # Estimate deviation from IF histogram
            ...
            # Rough baud estimate via run-lengths
            if deviation > 5000:  # likely FSK
                thr = np.median(inst_freq)
                bits = (inst_freq > thr).astype(int)
            else:  # likely OOK/ASK
                env = np.abs(analytic[1:])
                thr = np.median(env)
            ...
            # Modulation classification via amplitude kurtosis
            ...
            self._blind_errors += 1
            if self._blind_errors <= 5 or self._blind_errors % 100 == 0:
                logger.warning(f"Blind feature extraction error (count={self._blind_errors}): {e}")
            return None
    ...
    def reprocess_all_unknown_signals(self, db_connection, batch_size: int = 100) -> Dict:
        ...
        try:
            ...
            cursor.execute("""
                SELECT id, timestamp, frequency, signal_strength, snr, modulation_type,
                       baud_rate, packet_length, pattern_signature, raw_samples, retry_count
            ...
            for i in range(0, total_count, batch_size):
                ...
                logger.info(f"Processing batch {batch_num}/{(total_count + batch_size - 1) // batch_size} ({len(batch)} signals)")
                        ...
                        for strategy_idx, strategy in enumerate(self.reprocessor.retry_strategies):
                            ...
                            if result:
                                ...
                                cursor.execute("""
                                    ...
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)
                                """, (
                                    result.tpms_id, result.timestamp, result.frequency,
                                    result.signal_strength, result.snr, result.pressure_psi,
                                    result.temperature_c, result.battery_low, result.protocol,
                                    result.raw_data, result.confidence, retry_count + 1
                                ))

                                # Mark as decoded
                                cursor.execute("""
                                    UPDATE unknown_signals
                                    SET decoded = 1, decoded_protocol = ?, decoded_at = ?
                                    WHERE id = ?
                                ...
                                cursor.execute("""
                                    ...
                                    VALUES (?, ?, ?, ?, 1, ?, ?, ?, ?)
                                """, (
                                    signal_id, strategy_name, retry_count + 1, time.time(),
                                    result.protocol, result.tpms_id, result.confidence,
            ...
            stats['success_rate'] = (stats['total_recovered'] / stats['total_processed'] * 100) if stats['total_processed'] > 0 else 0
...
""")
    ...
    def _reprocess_samples(self, iq_samples: np.ndarray, frequency: float, relaxed: bool = False) -> Optional[TPMSSignal]:
    ...
    def _reprocess_with_symbol_rate(self, iq_samples: np.ndarray, frequency: float, symbol_rate: int) -> Optional[TPMSSignal]:
    ...
    def start_evolution_worker(db_connection, decoder: TPMSDecoder, check_interval: int = 60):
        """
        Start a separate evolution worker thread for continuous learning
        """
                        ...
                        if unknown_signals:
                            ...
                            for sig_data in unknown_signals:
                                unknown = UnknownSignal(
                                    ...
                                    raw_samples=np.frombuffer(sig_data['raw_samples'],
                                                              dtype=np.complex64) if sig_data.get(
    ...
    if __name__ == "__main__":
        logger.info("TPMS Intelligent Decoder - Ready for integration")
        logger.info("Use create_decoder_with_learning() to instantiate with database")
